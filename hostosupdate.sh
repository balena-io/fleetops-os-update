...
function pre_update_fix_bootfiles_hook {
    ...
    curl --retry 9999 -f -s -L -o "$bootfiles_temp" https://raw.githubusercontent.com/resin-os/resinhup/77401f3ecdeddaac843b26827f0a44d3b044efdd/upgrade-patches/0-bootfiles || log ERROR "Couldn't download fixed '0-bootfiles', aborting."
...
function in_container_hostapp_update {
    ...
    tmppipe=$(mktemp -u)
    mkfifo "${tmppipe}"
    ${DOCKER_CMD} save "${update_package}" > "${tmppipe}" &
    ...
    volumes_args+=("-v" "${tmppipe}:/resinos-image.docker")
    ...
    case $binary_type in
        arm|x86)
            ...
            for binary in $tools_binaries; do
                ...
                curl --retry 9999 -f -s -L -o $tools_path/$binary $download_uri/$binary || log ERROR "Couldn't download tool from $download_uri/$binary, aborting."
...
if version_gt "$VERSION_ID" "2.0.6" &&
    version_gt "2.3.0" "$VERSION_ID"; then
        ...
        curl --retry 9999 -f -s -L -o $tools_path/resin-device-progress $download_url || log WARN "Couldn't download tool from $download_url, progress bar won't work, but not aborting..."
